...
class TalkerGraph:
    ...
    def _init_cache_layers(self):
        ...
        for layer in self.static_cache.layers:
            if not layer.is_initialized:
                # backport: transformers 4.x 的 StaticLayer.lazy_initialization 只接受 1 个参数
                try:
    ...
    def prefill_kv(self, past_key_values):
        ...
        for li in range(self.num_layers):
            # backport: transformers 4.57 的 DynamicCache.__getitem__ 返回 (k, v)；
            # 5.x 改为 .keys/.values 属性。这里两种都兼容。
            try:
